#!/usr/bin/env python
"""
Evaluate HunyuanWorld-Mirror on RealEstate10K test scenes using 8-view evaluation setup.
...
CUDA_VISIBLE_DEVICES=0 python evaluate_re10k.py --save_splats --render_hires --max_scenes 1
"""
...
ORI_IMAGE_SHAPE = (720, 1280)  # Original Re10K image shape (H, W)
...
def load_eval_index(index_path: str) -> dict:
    """Load evaluation index: {scene_key: {context: [...], target: [...]}}."""
...
def load_chunks_index(data_root):
...
def convert_poses(poses):
    """Convert .torch poses to c2w (OpenCV) and normalized intrinsics.
...
def apply_scale_normalization(c2w: torch.Tensor, context_indices: list):
    """Divide all translations by the baseline between the first two context views.

    Matches Re10K_NVStest._fetch_views() scale normalization.
    """
    t0 = c2w[context_indices[0], :3, 3]
    t1 = c2w[context_indices[1], :3, 3]
    scale = (t0 - t1).norm().item()
    if scale < 1e-8:
        scale = 1.0
...
def load_scene_data(chunk_path, scene_key, context_indices, target_indices,
    ...
    # Convert all poses
    c2w_all, intrinsics_all = convert_poses(cameras)
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate WorldMirror on Re10K (8-view eval setup)")
    parser.add_argument("--dataset_root", type=str, default="/media/stefano/0D91176038319865/data/re10k/test")
    parser.add_argument("--eval_index", type=str,
                        default="re10k_start_0_distance_200_ctx_8v_tgt_8v.json")
    parser.add_argument("--worldmirror_path", type=str, default="/home/stefano/Codebase/HunyuanWorld-Mirror")
    parser.add_argument("--output_dir", type=str, default="output/worldmirror_re10k_8v")
    ...
    parser.add_argument("--skip_existing", type=str, default=None,
                        help="Path to existing output dir; skip scenes that have both low_res/ and high_res/ results there")
    ...
    scene_to_chunk = load_chunks_index(dataset_root)
    ...
    if args.skip_existing is not None:
        existing_root = Path(args.skip_existing)
        already_done = set()
        for scene in available_scenes:
            if (existing_root / "low_res" / scene).exists() and \
               (existing_root / "high_res" / scene).exists():
                already_done.add(scene)
        available_scenes = [s for s in available_scenes if s not in already_done]
        print(f"Skipped {len(already_done)} already-processed scenes, {len(available_scenes)} remaining")
    ...
    for scene_idx, scene_key in enumerate(tqdm(available_scenes, desc="Evaluating")):
        ...
        # Scale normalization (baseline between first two context views, matching Re10K_NVStest)
        ...
        # Resize rendered images to eval resolution for metric computation
    ...
    print(f"\n{'='*60}")
    print(f"WorldMirror Re10K 8-view {H_eval}x{W_eval} [{mode_label}] ({len(all_psnr)} scenes):")
